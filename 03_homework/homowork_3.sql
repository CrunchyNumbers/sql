...
SELECT cp.customer_id, c.customer_last_name, c.customer_first_name, ROUND(SUM(cp.quantity * cp.cost_to_customer_per_qty),2) AS total_spent
...
--Altrnatively, to output all purchase details and requested columns with month and year, we may run the following query:
